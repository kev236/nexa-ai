...
export interface ApprovalStore {
  ...
  /**
   * resolvedBy is omitted only for step 18's auto-approve path (every
   * non-money action, by default) — "nobody but the owner approves
   * anything" (readme.md) still holds there: the owner set this policy
   * themselves, so there's no per-instance human resolvedBy to record.
   * Every human-driven resolution (the dashboard's Approve/Deny, or an
   * owner-approved money-spending action) always passes a real owner id.
   */
  ...
  /** Most recent first — lets a caller see how an approval was actually resolved (owner vs. auto). */
