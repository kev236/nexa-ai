...
import { sendTalkMessageAction, talkLogoutAction } from '@/app/talk/actions'

export function TalkConsole({ name }: { name: string }) {
  const {
    messages,
    pending,
    error,
    ...
    send,
    toggleListening,
  } = useNexaChat(`Hey ${name}, it's Nexa. What's up?`, sendTalkMessageAction)
  ...
  return (
    <>
      <div className="page-header">
        <h1>Talking with {name}</h1>
        <p className="subtitle">
          Not business data here — just a chat.{' '}
          <button type="button" className="link-button" onClick={() => talkLogoutAction()}>
            Not {name}?
          </button>
        </p>
      </div>

      <div className="chat-console deck-enter">
